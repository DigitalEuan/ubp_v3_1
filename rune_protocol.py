...
class RuneProtocol:
    ...
    def _calculate_operation_nrci(self, initial_state: GlyphState, 
        ...
        final_coherence = 1.0 - np.var(final_state.state_vector) / (final_mean_sq + 1e-10)
        ...
        if len(initial_state.state_vector) > 0 and len(final_state.state_vector) > 0:
            ...
            if min_len > 1: # Require at least 2 elements for meaningful vector comparison
                initial_norm_vec = initial_state.state_vector[:min_len]
                final_norm_vec = final_state.state_vector[:min_len]
                
                initial_vec_norm = np.linalg.norm(initial_norm_vec)
                final_vec_norm = np.linalg.norm(final_norm_vec)

                if initial_vec_norm > 1e-10 and final_vec_norm > 1e-10:
                    initial_norm_vec = initial_norm_vec / initial_vec_norm
                    final_norm_vec = final_norm_vec / final_vec_norm
                    
                    information_preservation = abs(np.dot(initial_norm_vec, final_norm_vec))
                else: # One or both normalized vectors are zero, no meaningful dot product
                    information_preservation = 0.5 # Neutral value
            else: # Vector length is 0 or 1, dot product is not indicative of preservation
                information_preservation = 0.5 # Neutral value
    ...
    def _update_coherence_pressure_state(self, new_pressure: float):
        ...
        if len(self.coherence_pressure_state.pressure_history) > 10:
            ...
            if pressure_mean > 1e-10: # Avoid division by zero
                stability = 1.0 - pressure_variance / (pressure_mean**2)
                self.coherence_pressure_state.stability_index = max(0.0, min(1.0, stability))
            else:
                self.coherence_pressure_state.stability_index = 0.0 # If mean is zero, variance is zero, but coherence implies non-zero activity
