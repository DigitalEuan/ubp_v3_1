"""
UBP Framework v3.1.1 - BitTime Mechanics
...
class BitTimeMechanics:
    ...
    def create_bittime_state(self, realm: str, time_seconds: float = None) -> BitTimeState:
        ...
        self.current_time_state = bittime_state
        self.temporal_history.append(bittime_state)
    ...
    def export_state(self) -> Dict[str, Any]:
        """Exports the current state of BitTime Mechanics to a dictionary for serialization."""
        return {
            "current_time_state": asdict(self.current_time_state) if self.current_time_state else None,
            "temporal_history_count": len(self.temporal_history), # Don't export full history, can be large
            "temporal_history_last_5": [asdict(s) for s in self.temporal_history[-5:]],
            "planck_time_precision": self.get_planck_time_precision()
        }
    
    def import_state(self, state_data: Dict[str, Any]):
        """Imports the state into BitTime Mechanics from a dictionary."""
        if state_data.get("current_time_state"):
            self.current_time_state = BitTimeState(**state_data["current_time_state"])
        self.temporal_history = [BitTimeState(**s) for s in state_data.get("temporal_history_last_5", [])]
        # Full history is not imported to save memory; would need a more complex solution for that.
